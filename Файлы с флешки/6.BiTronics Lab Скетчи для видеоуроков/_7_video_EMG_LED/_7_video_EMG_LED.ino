...
#define threshold 160                        // порог срабатывания светодиода (может требовать подстройки под конкретного человека)
#define LED 7                                // определим номер вывода Arduino, к которому подключается светодиод
